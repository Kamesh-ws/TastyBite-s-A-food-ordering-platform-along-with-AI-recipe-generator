...
const SplMenuUpdate = () => {
    ...
    useEffect(() => {
        const fetchMenu = async () => {
            try {
                const response = await axios.get(`http://localhost:4000/api/restaurant/splmenu/${menuId}`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:4000/api/special-dish/${menuId}`, formData, {
                headers: {
                    token: `${token}`, // Include Authorization header
